...
function initStars() {
  ...
  for (let i = 0; i < 200; i++) {
    stars.push({
      ...
      color: ["#ffffff", "#FFD93D", "#4ECDC4", "#A855F7"][
...
function drawStars() {
  ...
  stars.forEach((s) => {
    const alpha =
      0.3 + 0.7 * (0.5 + 0.5 * Math.sin(t * s.speed * 10 + s.phase));
    ctx.beginPath();
...
// ═══════════════════════════════════════════
const ROCKET_MESSAGES = ["Hello! 👋", "To start your voyage,\nscroll down! ⬇️"];
...
function updateNavForUser() {
  ...
  if (currentUser) {
    nav.innerHTML = `
      <div class="user-chip">
        <div class="user-avatar">${currentUser.name
          .charAt(0)
          .toUpperCase()}</div>
...
function buildSlides() {
  ...
  const greetingHTML = name
    ? `👋 <b>Hello, ${name}!</b>`
    : isGuest
    ? `👋 <b>Hello, curious explorer!</b>`
    : `👋 <b>Hello, young explorer!</b>`;

  const slidesText = [
    `${greeting} I'm Commander Lidika, and I've traveled all the way from the Moon just to meet you! Welcome to Exploring the Moon with Artmeis — your mission to the Moon starts right here!`,
    ...
    `That's why Exploring the Moon with Artmeis turns the entire Moon mission into YOUR adventure! You'll engineer the rocket, train like a real astronaut, do actual calculations to launch and land, and even explore the lunar surface — all inside an interactive game built just for you!`,
    `And the best part? The site watches how YOU perform — tracking your strengths, spotting where you need help, and giving you a personal astronaut score after every mission. By the time you're done, you'll think like a real space engineer!`,
  ];

  const slidesHTML = [
    `${greetingHTML} I'm Commander Lidika, and I've traveled all the way from the Moon just to meet you! Welcome to <span class="highlight">Exploring the Moon with Artmeis</span> — your mission to the Moon starts right here! 🌙`,
    ...
    `🚀 That's why <span class="highlight">Exploring the Moon with Artmeis</span> turns the Moon mission into YOUR adventure! Engineer the rocket, train like an astronaut, do real calculations to launch and land, and explore the lunar surface — all inside an <b>interactive game built just for you!</b> 🌕`,
    `🏆 And the site <span class="highlight">watches how YOU perform</span> — tracking your strengths, spotting where you need help, and giving you a <b>personal astronaut score</b> after every mission. By the end, you'll think like a real space engineer! 💫`,
  ];
...
function speakSlide(idx) {
  ...
  function pickVoice() {
    ...
    return (
      v.find((v) =>
        /samantha|victoria|karen|zira|google us english/i.test(v.name)
      ) ||
      v.find((v) => /female|woman/i.test(v.name) && /en/i.test(v.lang)) ||
      v.find((v) => /en/i.test(v.lang)) ||
      v[0] ||
      null
    );
  }
...
function openModal(mode) {
  ...
  if (mode === "signup") {
    ...
    document.getElementById("modal-switch").innerHTML =
      "Already have an account? <a onclick=\"switchModal('login')\">Log In</a>";
    ...
    document.getElementById("modal-switch").innerHTML =
      "Don't have an account? <a onclick=\"switchModal('signup')\">Sign Up</a>";
    submitBtn.textContent = "Let's Go! 🚀";
    submitBtn.onclick = handleLogin;
  }
  setTimeout(() => {
    const focus = document.getElementById(
      mode === "signup" ? "modal-name" : "modal-user"
    );
...
document.addEventListener("keydown", (e) => {
  ...
  if (
    ...
    if (modalMode === "signup") handleSignup();
    else handleLogin();
  }
});
